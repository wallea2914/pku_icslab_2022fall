 ...
 * 这个shell能够执行内置命令或者代理用户执行用户程序,有"tsh>"提示符
 * 
 * 内置命令有：
 * quit                 --退出shell
 * jobs                 --打印当前所有作业
 * bg job               --让job后台运行,job用PID或者JID表示
 * fg job               --让job前台运行
 * kill job             --终止job,可以用负号表示进程组
 * nohup [command]      --让[command]中的命令忽略SIGHUP信号
 * 
 * 执行用户程序时，命令行输入可执行文件路径名和参数,若在命令行结尾有
 * "&"字符代表该程序在后台运行,否则程序在前台运行至结束
 *
 */
#include <assert.h>
#include <stdio.h>
...
/*几个eval函数的helper func*/
void wait_fg(sigset_t mask);                      //等待前台进程
int handle_argv(struct job_t **job, char **argv); //为fg、bg、kill处理第二个参数

/*
 * wait_fg - 等待前台进程结束
 * 
 * 参数：mask - 一个掩码，由调用者对应的掩码决定
*/

void wait_fg(sigset_t mask)
{
...
/**
 * handle_argv - 判断内置命令的参数是否正确，并处理
 * 
 * 参数：job - 二级指针，读取jid或者pid后方便将对应的job回写到指定位置
 *      argv - 参数表
 * 
 * 返回值：0 - 参数异常     1 - 参数合法
*/

int handle_argv(struct job_t **job, char **argv)
{
    if (argv[1] == NULL)
    {
        sio_puts(argv[0]);
...
void eval(char *cmdline)
{
    ...
    sigaddset(&mask_one, SIGCHLD); //只有child
    ...
    sigaddset(&mask_three, SIGTSTP); //三种信号
    ...
    if (!tok.outfile) //没有重定向就是stdout
        fout = STDOUT_FILENO;
    else if ((fout = open(tok.outfile, O_RDWR | O_CREAT)) < 0)
    {
        sio_puts("failed to open or create file ");
        sio_puts(tok.outfile);
        sio_puts("\n");
    }
    if (!tok.infile) //没有重定向就是stdin
        fin = STDIN_FILENO;
    else if ((fin = open(tok.infile, O_RDONLY)) < 0)
    {
        sio_puts("failed to open or create file ");
        sio_puts(tok.infile);
        sio_puts("\n");
    ...
    if (tok.builtins != BUILTIN_NONE)
    {
        ...
        {
            struct job_t *job;
            if (handle_argv(&job, tok.argv) == 0) //参数不对
                return;
        ...
        case BUILTIN_NOHUP:
        {
            ...
            sigprocmask(SIG_BLOCK, &hup, &mask_prev);
            sigprocmask(SIG_BLOCK, &mask_one, &tmp);
            pid = fork();
            if (pid == 0)
            {
...
void sigchld_handler(int sig)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0)
    {
        ...
        if (WIFSIGNALED(status)) //终止
        {
            sio_put("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
            deletejob(job_list, pid);
        }
        if (WIFCONTINUED(status)) //被重启
        {
            struct job_t *job = getjobpid(job_list, pid);
            job->state = BG;
        }
